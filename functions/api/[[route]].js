...
// Routes: GET /api/state, POST /api/claim, POST /api/vote,
//         POST /api/bio, GET /api/bio/:id, POST /api/optout,
//         POST /api/reset
// ═══════════════════════════════════════════════════════════
...
async function handleGetState(env) {
  const [claimList, voteList, optoutList, bioList] = await Promise.all([
    env.KV.list({ prefix: 'claim:' }),
    env.KV.list({ prefix: 'votes:' }),
    env.KV.list({ prefix: 'optout:' }),
    env.KV.list({ prefix: 'bio:' }),
  ]);
  const claims = claimList.keys.map(k => k.name.substring(6));
  const optouts = optoutList.keys.map(k => k.name.substring(7));
  const bio_ids = bioList.keys.map(k => k.name.substring(4));

  const votes = {};
  await Promise.all(
    voteList.keys.map(async (k) => {
      const voterId = k.name.substring(6);
      const val = await env.KV.get(k.name);
      if (val) {
        try { votes[voterId] = JSON.parse(val); } catch {}
      }
    })
  );
  return json({ claims, votes, optouts, bio_ids });
}
...
async function handleClaim(request, env) {
  ...
  await env.KV.put(`claim:${id}`, JSON.stringify({ token, at: Date.now() }));
...
async function handleVote(request, env) {
  ...
  let votes = {};
  if (currentStr) {
    try { votes = JSON.parse(currentStr); } catch {}
  }
  ...
  if (isEmpty) {
    ...
    if (Array.isArray(value)) {
      ...
      votes[pool_key] = value.filter(v => typeof v === 'string');
...
export async function onRequest(context) {
  ...
  if (!env.KV) {
    return json({ error: 'KV binding missing.​​​​​​​​​​​​​​​​
